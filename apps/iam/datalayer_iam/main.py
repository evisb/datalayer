...
def signup_confirm():
    ...
    return redirect(os.getenv('DLA_IAM_UI_REDIRECT') + '/join/confirm')
...
#        errors.append('Username `{}` is not available'.format(username))
    ...
    if not res:
        errors.append('Something went wrong. Please contact support@datlaayer.io')
        return jsonify({ 
            'success': res,
            'errors': errors,
            })
...
def forgot_password_confirm():
    ...
    ldap_conn.modify(user, { 'userPassword': [(MODIFY_REPLACE, [password])] })
    logging.info('Welcome: %s' % username)
    return redirect(os.getenv('DLA_IAM_UI_REDIRECT') + '/forgotpassword/confirm')
