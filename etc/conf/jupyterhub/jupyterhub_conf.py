...
pjoin = os.path.join
log_dir = os.path.join(os.environ['$DLAHOME' + '/var/log')
runtime_dir = os.path.join(os.environ['$DLAHOME' + '/srv/jupyterhub')

# --- Common ---
c.JupyterHub.confirm_no_ssl = False
c.ConfigurableHTTPProxy.auth_token = pjoin(runtime_dir, 'datalayer_auth_token')
c.JupyterHub.cookie_secret_file = pjoin(runtime_dir, 'datalayer_cookie_secret')
...
c.DockerSpawner.image = 'datalayer/jupyterlab-hub:0.0.1'
...
c.JupyterHub.services = [
    {
        'name': 'cull-idle',
        'admin': True,
        'command': 'python {}/etc/services/jupyterhub/cull-idle/cull_idle_servers.py --timeout=3600'.format(os.environ['DLAHOME']).split(),
    },
]
